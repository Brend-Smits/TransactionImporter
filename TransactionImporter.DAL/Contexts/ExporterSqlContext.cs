    ...
    public class ExporterSqlContext:IExporterContext
    {
        ...
        public List<CustomerInfo> GetEuCustomers()
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    SqlCommand selectUuid = new SqlCommand("SELECT [CustomerInfoUUID] FROM [Transaction] WHERE (UploadId) LIKE @UploadId AND [Country] IN (SELECT [CountryCode] FROM [CountryContinent] WHERE [Continent] = 'EU')", connection);
                    selectUuid.Parameters.AddWithValue("UploadId", uploadId);
                    using (SqlDataReader uuidReader = selectUuid.ExecuteReader())
                    {
                        while (uuidReader.Read())
